<x-admin-layout>
    <x-slot name="title">New Word List</x-slot>
    ...
    <div class="max-w-xl">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100">
            <div class="px-6 py-5 border-b border-slate-100">
                <h2 class="font-extrabold text-slate-800">Create Word List</h2>
                <p class="text-slate-500 text-sm mt-0.5">After creating the list you can add words to it.</p>
            </div>

            <form method="POST" action="{{ route('admin.word-lists.store') }}" class="p-6 space-y-5"
                x-data="{ label: '', slug: '' }"
                @input.once="null">
                ...
                <div>
                    ...
                    <input type="text" name="label" value="{{ old('label') }}" required
                        ...
                        placeholder="e.g. Year 1 — Starter"
                ...
                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-1.5">
                        ...
                        <span class="text-slate-400 font-normal text-xs ml-1">— unique identifier used in URLs and game settings</span>
                    </label>
                ...
                <div class="grid grid-cols-2 gap-5">
                    <div>
                        ...
                        <input type="number" name="sort_order" value="{{ old('sort_order', 99) }}" min="0"
                            class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-400 focus:border-indigo-400 outline-none">
                        <p class="text-xs text-slate-400 mt-1">Lower = appears first in dropdowns</p>
                    </div>
                    ...
                    <div class="flex items-center pt-6">
                        <label class="flex items-center space-x-3 cursor-pointer">
                            ...
                            <span class="text-sm font-bold text-slate-700">Active (visible in spelling game)</span>
                        </label>
                    </div>
                </div>
